...
namespace Assets.Scripts.SUMOConnectionScripts.Maps
{
    ...
    /// <summary>
    /// Code from ditzel's open unity tutorials, source: https://gist.github.com/ditzel/194ec800053ce7083b73faa1be9101b0
    /// </summary>
    ...
    public class KdTree<T> : IEnumerable<T>, IEnumerable where T : Component
    {
        ...
        public IEnumerable<T> FindClosest(Vector3 position, int amount)
        {
            List<T> closest = new List<T>();
            while(closest.Count<amount)
            {
                T nextClosest= _findClosest(position, null, closest);
                if (nextClosest == null)
                {
                    return closest;
                }
                closest.Add(nextClosest);
            }
            return closest;
        }


        protected T _findClosest(Vector3 position, List<T> traversed = null, List<T> ignorelist = null)
        {
            if (_root == null)
                return null;
            ...
            while (openCur < _open.Length && _open[openCur] != null)
            {
                ...
                if (nodeDist < nearestDist && (ignorelist==null||!ignorelist.Contains(current.component)))
                {
